...
TABLES = {
    ...
    "damage_reports": os.getenv("TABLE_DAMAGE_REPORTS", "baocauhahi"),
...
class LocalNodeStorage:
    def __init__(self, root_dir: str):
        ...
        self.meta_file = self.root_dir / "meta.json"set
    ...
    def _save_meta(self, meta: dict):
        self.meta_file.write_text(stable_json(meta), encoding="utf-8")
...
class RentalAppService:
    ...
    def overview(self) -> dict:
        vehicles = self.t("vehicles").select("*").order("id", desc=True).limit(10).execute().data
        bookings = self.t("bookings").select("*").order("id", desc=True).limit(10).execute().data
        contracts = self.t("contracts").select("*").order("id", desc=True).limit(10).execute().data
        transactions = self.t("transactions").select("*").order("id", desc=True).limit(10).execute().data
        return {
            "vehicles": vehicles,
            "bookings": bookings,
            "contracts": contracts,
...
if not SUPABASE_URL or not SUPABASE_KEY:
    # App van khoi dong duoc de hien HTML, nhung API se bao loi ro rang.
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("server_demo:app", host="0.0.0.0", port=8000, reload=True)
